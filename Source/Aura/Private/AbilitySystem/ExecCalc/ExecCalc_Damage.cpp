...
struct AuraDamageStatics
{
	...
	AuraDamageStatics()
	{
		//这个宏用于获取一个类（参数1）中的特定类（参数2），并根据该类和指定的属性源、快照与否来给声明中的Def赋值
...
{
	//该数组为GECalculation较UObject多出的唯一变量，此后可通过 AttemptCalculateMagnitude() 或 GetCapturedAttributeMagnitude() 读取这些属性
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	//本类由运行参数中获取到了源和目标的ASC，EffectSpec
	...
	//在实现多种伤害类型后，通过循环来把所有伤害类型标签和对应抗性的映射遍历一遍
	float Damage = 0.f;
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair  : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		//GESpec中存在局部变量TMap<FGameplayTag, float>	SetByCallerTagMagnitudes;
		//在GA触发后生成发射物时，在这个map中加入了伤害类型Tag和伤害数值的映射
	...
	//这个OutExecutionOutput是一个类似于Modifier数组，可以实现多参数修改
	//不难看出，这里只修改了IncomingDamage这一个属性值
